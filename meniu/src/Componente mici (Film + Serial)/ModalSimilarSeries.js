...
const ModalSimilarSeries = ({ id, name, poster_path, vote_average, first_air_date }) => {
    return (
        <div>
            {first_air_date ?
                ...
                <div className="movie-header-similar-filme">
                    ...
                    <span style = {{position: 'absolute', top: '4px', left: '4px', fontSize: '17px', fontFamily: 'Poppins', fontWeight: 'lighter'}} className="badge rounded-pill bg-danger">{vote_average.toString().substr(0, 3)}</span>
                    <div className="movie-info-similar-filme">
                        <h3 style = {{fontSize: 15, color: 'black'}}>{name} ({first_air_date.substring(0,4)})</h3>
                    </div>
                </div>
            </div>
            : ''}
