...
const ActorFilme = ({ title, poster_path, backdrop_path, id }) => {
    ...
    return (
        <div className='movie-recomandari'>
            ...
            <h4 style = {{marginLeft: '10px', marginTop: '5px', fontWeight: 'lighter'}}>Actorii pentru filmul <span style = {{color: 'black'}} className="badge bg-dark text-white">{title}</span></h4>  
            <div className="similarMovies">          
                {actors.length > 0 ? actors.map(actor => (
                    <ModalActors key={actor.id} {...actor}></ModalActors>
                )) : (
                    <h2 style = {{color: 'black', marginTop: '180px', marginLeft: '250px'}} className="no-movies">
                        <AiOutlineAlert style = {{width: '40px', height: '40px', marginTop: '-5px'}}></AiOutlineAlert> Ups... Nu exista momentan niciun actor de afisat pentru acest film
                        <p style = {{marginTop: '5px', fontSize: '25px', marginLeft: '430px'}}>Keep trying!</p>
                        <p style = {{fontWeight: 'lighter', fontStyle: 'italic', backgroundColor: 'transparent', color: 'white', marginBottom: '-1px', marginTop: '10px', marginLeft: '120px', fontSize: '20px'}} className="card-text">Daca nu te descurci, nu uita sa apelezi la meniul <button style = {{marginLeft: '5px', marginRight: '5px'}} type="button" className="btn btn-outline-warning">Ajutor</button> din partea de sus a paginii</p>
                    </h2>
                )}
